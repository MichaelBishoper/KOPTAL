...
import { getTenantByName, getTenantProducts, getTenantProfileImage, getTenants, shouldUseNativeImage, safeImageSrc, toCatalogCard } from "@/lib";
...
export default function TenantPage({ name, location }: TenantPageProps) {
  ...
  const selectedTenantImageRaw = getTenantProfileImage(selectedTenant);
  const selectedTenantImage = safeImageSrc(selectedTenantImageRaw);
