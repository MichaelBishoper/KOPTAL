...
import { saveUserProfileDraft, safeImageSrc } from "@/lib";
...
export function UserProfile({ user, userType, onLogout, loggingOut }: UserProfileProps) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // If updated user has an image, set a non-httpOnly cookie so UI can persist avatar across sessions.
      try {
        const normalized = (updatedUser as Record<string, unknown>).image_url ?? (updatedUser as Record<string, unknown>).image;
        if (typeof normalized === "string" && normalized) {
          document.cookie = `koptal_image=${encodeURIComponent(normalized)}; path=/; max-age=${60 * 60 * 24 * 30}`;
        }
      } catch {
        // no-op
      }
  ...
  const storedProfileImage =
    ...
    undefined;
  const profileImageSrcRaw =
    (isEditing ? localPreviewImage ?? draft.image : undefined) ||
    storedProfileImage ||
    "/product-placeholder.jpg";
  const profileImageSrc = safeImageSrc(profileImageSrcRaw);
